...
export function ItineraryPanel({
  ...
  // Check if the current itinerary is already saved
  useEffect(() => {
    const checkIfSaved = async () => {
      ...
      try {
        ...
        const savedItineraries = await response.json();
        // Check if current itinerary matches any saved itinerary (compare by summary)
        const alreadySaved = savedItineraries.some(
          (saved: any) => saved.itinerary.summary === itinerary.summary
        );
      ...
      <Card className="h-full flex flex-col overflow-hidden">
        ...
        <CardContent className="flex-1 flex items-center justify-center">
          <p className="text-sm text-slate-500 italic">
            Complete your trip planning to generate an itinerary
          </p>
        </CardContent>
      </Card>
    );
    ...
    try {
      ...
      const response = await fetch(`/api/trips/${tripId}/itineraries`, {
        ...
        body: JSON.stringify({
          itinerary,
          tripContext,
          name: `${itinerary.summary.substring(0, 50)}${
            itinerary.summary.length > 50 ? "..." : ""
          }`,
        }),
      });
          ...
          <div className="flex gap-2 flex-wrap mt-4">
            {tripContext.trip?.preferences?.pace && (
              <Badge variant="secondary">
                {tripContext.trip.preferences.pace} pace
              </Badge>
            )}
            {tripContext.trip?.budget?.level && (
              <Badge variant="secondary">
                {tripContext.trip.budget.level} budget
              </Badge>
            )}
            {tripContext.trip?.preferences?.dietary?.length > 0 && (
              <Badge variant="secondary">
                {tripContext.trip.preferences.dietary.join(", ")}
              </Badge>
            )}
            {tripContext.trip?.constraints?.avoid?.some((item) =>
        ...
        <ScrollArea className="h-full">
          <div className="space-y-4 md:space-y-6 pr-3 md:pr-4">
            ...
            <div className="space-y-3">
              ...
              {itinerary.days && itinerary.days.length > 0 ? (
                ...
                <div className="text-center py-8 text-slate-500">
                  <p>
                    No itinerary days found. Please generate an itinerary first.
                  </p>
                </div>
              )}
            </div>
            ...
            {itinerary.alternativeOptions &&
              countAlternatives(itinerary.alternativeOptions) > 0 && (
                <>
                  <Separator />
                  <Accordion type="single" collapsible className="w-full">
                    <AccordionItem value="alternatives">
                      <AccordionTrigger className="text-sm font-semibold hover:no-underline">
                        ✨ Alternative Options (
                        {countAlternatives(itinerary.alternativeOptions)})
                      </AccordionTrigger>
                      <AccordionContent>
                        <div className="space-y-4">
                          {itinerary.alternativeOptions.transport &&
                            itinerary.alternativeOptions.transport.length >
                              0 && (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {itinerary.alternativeOptions.transport.map(
                                    (option, idx) => (
                          ...
                          {itinerary.alternativeOptions.stays &&
                            itinerary.alternativeOptions.stays.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {itinerary.alternativeOptions.stays.map(
                                    (option, idx) => (
                                      <Card key={idx} className="p-3 text-sm">
                                        {renderObject(option)}
                                      </Card>
                          ...
                          {itinerary.alternativeOptions.dining &&
                            itinerary.alternativeOptions.dining.length > 0 && (
                              <div>
                                ...
                                <div className="space-y-2">
                                  {itinerary.alternativeOptions.dining.map(
                                    (option, idx) => (
                                      <Card key={idx} className="p-3 text-sm">
                                        {renderObject(option)}
                                      </Card>
...
function renderObject(obj: unknown): React.ReactNode {
  ...
  if (typeof obj === "object" && obj !== null) {
    ...
    const hasSimpleValues = entries.every(
...
function countAlternatives(alternatives: {
  transport?: unknown[];
  stays?: unknown[];
  dining?: unknown[];
}): number {
  ...
  itinerary.days.forEach((day) => {
    ...
    if (day.accommodation) {
      ...
      if (day.accommodation.estimatedCost)
        md += ` - ${day.accommodation.estimatedCost}`;
      md += "\n\n";
    }
